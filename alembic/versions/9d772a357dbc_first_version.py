...
def upgrade():
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint('id', name=op.f(
            'pk_classification_training_data'))
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['task_id'], ['task.id'], name=op.f(
            'fk_annotation_request_task_id_task')),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], name=op.f(
            'fk_annotation_request_user_id_user')),
        sa.PrimaryKeyConstraint(
            'id', name=op.f('pk_annotation_request'))
    )
    ...
    op.create_table(
        ...
        sa.Column('context', sa.JSON(), nullable=True),
        sa.ForeignKeyConstraint(['user_id'], ['user.id'], name=op.f(
            'fk_classification_annotation_user_id_user')),
        sa.PrimaryKeyConstraint('id', name=op.f(
            'pk_classification_annotation'))
    )
    ...
    op.create_table(
        ...
        sa.Column('classification_training_data_id',
                  sa.Integer(), nullable=True),
        sa.Column('config', sa.JSON(), nullable=True),
        sa.Column('task_id', sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(['classification_training_data_id'], ['classification_training_data.id'], name=op.f(
        ...
        sa.UniqueConstraint('uuid', 'version',
                            name='_uuid_version_uc')
    )
    with op.batch_alter_table('model', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_model_type'), [
                              'type'], unique=False)
        batch_op.create_index(batch_op.f('ix_model_uuid'), [
                              'uuid'], unique=False)
        batch_op.create_index(batch_op.f('ix_model_version'), [
