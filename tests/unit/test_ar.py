...
def test_assign_round_robin():
    ...
    assert per_anno_queue == {"u1": ["a", "c"], "u2": ["b"]}
...
def test_assign_unlimited_budget():
    ...
    assert per_anno_queue == {"u1": ["a", "b", "c"], "u2": ["a", "b", "c"]}
...
def test_assign_limited_by_max_per_annotator():
    ...
    assert per_anno_queue == {"u1": ["a", "b"], "u2": ["a", "b"]}
...
def test_assign_blacklist():
    ...
    def blacklist_fn(dp, anno):
        return dp == "a" and anno == "u1"

    per_anno_queue = ar._assign(
        datapoints,
        annotators,
        max_per_annotator=999,
        max_per_dp=999,
        blacklist_fn=blacklist_fn,
    )

    assert per_anno_queue == {"u1": ["b", "c"], "u2": ["a", "b", "c"]}
...
def populate_db(dbsession):
    ...
    def create_anno(user: User, entity: str, label: str, value: int):
        instance = ClassificationAnnotation(
            entity_type="blah", entity=entity, label=label, value=value, user_id=user.id
        )
...
def test_build_blacklist_lookup(dbsession):
    ...
    bar_lookup = set({key("entity0", "bar", "user0")})
