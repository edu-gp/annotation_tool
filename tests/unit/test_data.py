...
from alchemy.ar.data import (
    EntityAndAnnotationValuePair,
    PrettyDefaultDict,
    _compute_kappa_matrix,
    _compute_num_of_annotations_per_value,
...
def test__compute_kappa_matrix():
    ...
    kappa_matrix = _compute_kappa_matrix(kappa_stats_raw_data=raw_data)
...
def _populate_annotation_data(dbsession):
    ...
    annotation1 = ClassificationAnnotation(
        value=1, user_id=user1.id, label=label1, entity_type=ENTITY_TYPE, entity=entity1
    )
    annotation2 = ClassificationAnnotation(
        value=1, user_id=user1.id, label=label1, entity_type=ENTITY_TYPE, entity=entity2
    )
    ...
    annotation4 = ClassificationAnnotation(
        value=1, user_id=user2.id, label=label1, entity_type=ENTITY_TYPE, entity=entity1
    )
    ...
    annotation6 = ClassificationAnnotation(
        value=-1,
        user_id=user3.id,
        label=label1,
        entity_type=ENTITY_TYPE,
    ...
    annotation7 = ClassificationAnnotation(
        value=-1,
        user_id=user3.id,
        label=label2,
        entity_type=ENTITY_TYPE,
    ...
    return user1, user2, user3, entity1, entity2, entity3, label1, label2, annotations


def test__compute_num_of_annotations_per_value(dbsession):
    user1, user2, user3, _, _, _, label1, label2, annotations = _populate_annotation_data(
        dbsession
    )
...
def test__compute_total_annotations(dbsession):
    ...
    for label in [label1, label2]:
        ...
        for annotation in annotations:
            if annotation.label == label:
                expected[
                    UserNameIdPair(annotation.user.username, annotation.user_id)
                ] += 1
...
def test__construct_kappa_stats_raw_data(dbsession):
    user1, user2, user3, entity1, entity2, entity3, label1, label2, _ = _populate_annotation_data(
        dbsession
    )
    ...
    assert res1 == {user1.username: [1, 1], user2.username: [1, -1]}
    ...
    assert res2 == {user1.username: [-1], user3.username: [-1]}
    ...
    kappa_raw_data = _construct_kappa_stats_raw_data(
        dbsession=dbsession, distinct_users={user1, user2, user3}, label=label1
    )
...
def _populate_annotation_requests(dbsession):
    ...
    context1 = {
        ...
        "score": 0.98,
        "source": "testing",
        "data": {"text": "Blah blah ...", "meta": {"name": "Blah", "domain": "blah"}},
        "pattern_info": {
            "tokens": ["Blah", "blah"],
            "matches": [(1, 2, "Blah")],
...
def test_compute_annotation_request_statistics(dbsession):
    task1, task2, user1, user2, requests = _populate_annotation_requests(dbsession)
...
def test_fetch_ar_ids(dbsession):
    task1, task2, user1, user2, requests = _populate_annotation_requests(dbsession)
    res = fetch_ar_ids(dbsession=dbsession, task_id=task1.id, username=user1.username)

    for request in requests:
        if request.task_id == task1.id and request.user.username == user1.username:
...
def test_fetch_annotated_ar_ids_from_db(dbsession):
    task1, task2, user1, user2, requests = _populate_annotation_requests(dbsession)
    res = fetch_annotated_ar_ids_from_db(
        dbsession=dbsession, task_id=task1.id, username=user1.username
    )

    for request in requests:
        if (
            request.task_id == task1.id
            and request.user.username == user1.username
            and request.status == AnnotationRequestStatus.Complete
        ):
            assert request.id in set(res)
        else:
            assert request.id not in set(res)
...
def test__construct_comparison_df(dbsession):
    ...
    dbsession.add_all([annotation1, annotation2, annotation3, annotation4, annotation5])
    dbsession.commit()

    comparison_df, id_df = _construct_comparison_df(
        dbsession=dbsession,
        label=label,
        users_to_compare=[user1.username, user2.username, user3.username],
    )
...
def test_delete_requests_under_task(dbsession):
    user1, user2, label, task1, task2, requests = _populate_common_request_data(
        dbsession
    )
...
def test_delete_requests_for_user_under_task(dbsession):
    ...
    assert _count_requests(dbsession, task_id=task1.id, user_id=user2.id) == 1
    delete_requests_for_user_under_task(
...
def _count_requests(dbsession, **kwargs):
    return (
        dbsession.query(func.count(AnnotationRequest.id)).filter_by(**kwargs).one()[0]
    )
