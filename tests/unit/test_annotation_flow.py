...
def test_annotation_flow(dbsession):
    ...
    req = AnnotationRequest(
        ...
        name="Is a Banana Sweet?",
        context={"text": "Studies have shown that banana is sweet when ripe."},
    )
    dbsession.add(req)
    dbsession.commit()
    ...
    reqs = (
        ...
        .join(User)
        .filter(AnnotationRequest.user == user)
        .all()
    )
    assert len(reqs) == 1
    ...
    assert user.classification_annotations.filter_by(entity=req.entity).count() == 0
    ...
    assert user.classification_annotations.filter_by(entity=req.entity).count() == 2
