...
from alchemy.train.no_deps.utils import gs_copy_file, gs_exists


class DeployedInferenceMetadata:
    def __init__(
        self, timestamp, model_uuid, model_version, label, threshold, dataset_name
    ):
...
def create_deployed_inference(metadata: DeployedInferenceMetadata) -> None:
    ...
    if not has_model_inference(uuid, version, dname):
        raise Exception(
            f"Inference result not present on GCS, "
...
def _message_constructor_alchemy_to_gdp(
    ...
    message_dict = {
        ...
        "path": {"inferences": prod_inference_url, "metadata": prod_metadata_url},
    }
...
def build_prod_inference_dataframe(pred_fname, raw_fname, threshold):
    ...
    if len(inf.probs) != len(raw):
        raise Exception(
            "Prediction and raw files are different in length"
