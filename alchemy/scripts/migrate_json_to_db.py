...
def _convert_single_annotation(anno, username):
    ...
    entity_type, entity = _make_company_entity(
        anno["req"]["data"]["meta"], text=anno["req"]["data"]["text"]
    )
...
def _convert_single_request_with_annotated_result(
    ...
    """
    entity_type, entity = _make_company_entity(
        req["data"]["meta"], text=req["data"]["text"]
    )
    ...
    if anno_from_file:
        ...
        logging.info(
            "Converted annotation {} with ar_id {}".format(annotation_id, req["ar_id"])
        )
    else:
        annotation_id = None
        logging.info(
            "No annotation results found for this request {}".format(req["ar_id"])
        )
...
if __name__ == "__main__":
    ...
    if drop:
        ...
        from alchemy.db.model import Base
    ...
    for task_uuid in tqdm(task_uuids):
        ...
        for username in usernames:
            convert_annotation_request_in_batch(
                task_uuid=task_uuid, task=task, username=username
            )
        ...
        for username in usernames:
            convert_annotation_result_in_batch(task_uuid=task_uuid, username=username)
