...
from alchemy.bg.jobs import export_new_raw_data as _export_new_raw_data
...
def export_new_raw_data():
    ...
    resp = {"error": None, "message": "Request not processed."}
...
def update_model_deployment_config():
    ...
    for model in models:
        ...
        model_deployment_config = (
            db.session.query(ModelDeploymentConfig)
            .filter(ModelDeploymentConfig.model_id == model.id)
            .one_or_none()
        )
        ...
        if model_deployment_config:
            ...
            model_deployment_config.is_selected_for_deployment = (
...
def _collect_model_data_rows(data_store):
    ...
    for label in set(labels):
        ...
        annotators_sorted = (
            db.session.query(
                User.username, func.count(ClassificationAnnotation.id).label("num")
            )
        ...
        label_owner_id = (
            db.session.query(LabelOwner.owner_id)
