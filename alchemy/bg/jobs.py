...
from alchemy.shared.file_adapters import file_exists, save_jsonl
...
def export_new_raw_data(
    model: Model, data_fname: str, output_fname: str, data_store: str, cutoff: float = 0.5,
):
    """Exports a model prediction in the raw data format.
    Note: This will NOT allow you to overwrite an existing file.
    """

    df = model.export_inference(data_fname, include_text=True, data_store=data_store)
    ...
    if file_exists(output_path, data_store=data_store):
    ...
    save_jsonl(output_path, data, data_store=data_store)
