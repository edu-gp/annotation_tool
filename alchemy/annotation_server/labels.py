...
from alchemy.db.model import db, fetch_labels_by_entity_type, save_labels_by_entity_type
...
def save_labels():
    ...
    try:
        logging.info(
            "Updating new labels {} for EntityType {}".format(
                str(new_labels), entity_type_name
            )
        )
        save_labels_by_entity_type(db.session, entity_type_name, list(new_labels))
